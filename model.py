...
class SelfAttentionFusion(nn.Module):
    def __init__(self, news_embedding_dim):
        super(SelfAttentionFusion, self).__init__()
        self.attention = nn.Linear(news_embedding_dim, 1)

    def forward(self, x):
        # Repeat candidate news to match the shape of positive samples
        
        weights = torch.softmax(self.attention(x), dim=1)
        news_representation = torch.sum(x * weights, dim=1)
...
class Model(nn.Module):
    def __init__(self, config: Config):
        ...
        if config.news_encoder == 'CNRCL':
            self.news_encoder = newsEncoders.CNRCL(config)
        ...
        if config.user_encoder == 'CNRCL':
            self.user_encoder = userEncoders.CNRCL(self.news_encoder, config)
    ...
    def initialize(self):
        ...
        #self.news_encoder_user.initialize()
