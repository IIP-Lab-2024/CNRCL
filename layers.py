...
class TopKPool(nn.Module):
    ...
    # graph   : [batch_size, node_num, node_num]
    # Output
    # pooled_feature : [batch_size, new_node_num, feature_dim]
    # pooled_graph   : [batch_size, new_node_num, new_node_num]
    def forward(self, feature, graph):
        scores = self.score_layer(feature)  # [batch_size, node_num, 1]
        scores = scores.squeeze(-1)  # [batch_size, node_num]

        k = int(self.ratio * feature.size(1))
        topk_scores, topk_indices = scores.topk(k, dim=1)  # [batch_size, new_node_num]

        batch_size, node_num, feature_dim = feature.size()
        batch_indices = torch.arange(batch_size).view(-1, 1).expand(-1, k).reshape(-1).to(feature.device)
        topk_indices_flat = topk_indices.view(-1)

        pooled_feature = feature[batch_indices, topk_indices_flat].view(batch_size, k, feature_dim)
        pooled_graph = graph[batch_indices, topk_indices_flat].transpose(1, 2)
        pooled_graph = pooled_graph[batch_indices, topk_indices_flat].transpose(1, 2)

        return pooled_feature, pooled_graph

# Create a simple model with GCN and TopKPool
class TopKPoolModel(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_dim, num_layers, ratio=0.5, dropout=0.1, residual=False, layer_norm=False):
        super(TopKPoolModel, self).__init__()
        self.gcn = GCN(in_dim, hidden_dim, hidden_dim, num_layers, dropout, residual, layer_norm)
        self.topkpool = TopKPool(hidden_dim, ratio)
        self.output_layer = nn.Linear(hidden_dim, out_dim)

    def forward(self, feature, graph):
        gcn_output = self.gcn(feature, graph)
        pooled_feature, pooled_graph = self.topkpool(gcn_output, graph)
        output = self.output_layer(pooled_feature)
        return output