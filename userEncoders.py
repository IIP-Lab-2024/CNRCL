...
class UserEncoder(nn.Module):
    ...
    def feature_fusion(self, history_embedding_uncategory, category, subCategory):
        ...
        history_embedding = torch.cat([history_embedding_uncategory, self.dropout(category_representation), self.dropout(subCategory_representation)], dim=2) # [batch_size, news_num, news_embedding_dim]
        return history_embedding

class CNRCL(UserEncoder):
    def __init__(self, news_encoder: NewsEncoder, config: Config):
        super(CNRCL, self).__init__(news_encoder, config)

        self.word_embedding_dim = config.word_embedding_dim
        self.hidden_dim = config.hidden_dim
        self.news_embedding_dim = config.hidden_dim * 4 + config.category_embedding_dim + config.subCategory_embedding_dim
        self.history_lstm = nn.LSTM(self.news_embedding_dim, self.news_embedding_dim, batch_first=True, bidirectional=False)
    ...
    def forward(self, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, user_category, user_subCategory, \
        ...
        # [batch_size, max_history_num, news_embedding_dim], [batch_size, max_history_num,hidden_dim * 4]
        ...
        #gcn_feature = self.graphSAGE(history_embedding,user_history_graph) + history_embedding  # [batch_size, max_history_num + category_num, news_embedding_dim]
