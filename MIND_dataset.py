...
class MIND_Train_Dataset(data.Dataset):
    def __init__(self, corpus: MIND_Corpus):
        ...
        self.similarity_num = corpus.similarity_num
        self.miss_num = 0
        self.ID_news_dict = corpus.ID_news_dict  # id->news_id
        self.retain = corpus.retain
        #####################################

    def find_key_for_value(self,dictionary, target_value):
    ...
    def negative_sampling(self,epoch, rank=None):
        self.miss_num=0
        news_similarities = {}
        with open('./similarities_small.csv', 'r') as f:
        ...
        for i, train_behavior in enumerate(self.train_behaviors):
            ...
            #print(f"postive {train_behavior[3]}")
            ...
            if news_num > 46:
                index = news_num // 2
                if epoch <= 3:
                    negative_samples = negative_samples[:index]  # retain 50%
                elif 3 < epoch <= 6:
                    negative_samples = negative_samples[:index + news_num * 1 // 8]  # retain 62.5%
                elif 6 < epoch <= 9:
                    negative_samples = negative_samples[:index + news_num * 2 // 8]  # retain 75%
                elif 9 < epoch <= 12:
                    negative_samples = negative_samples[:index + news_num * 3 // 8]  # retain 87.5%
                elif 12 < epoch <= 16:
                    negative_samples = negative_samples  # retain (100%)
            news_num = len(negative_samples)
                ...
                for j in range(self.negative_sample_num):
                    while True:
                        ...
                        if k not in used_negative_samples:
                            ...
                            break
            # ****************************************************************************** #
            news_ID=self.ID_news_dict[train_behavior[3]]
            n=0
            if news_ID in news_similarities:
               similar_news_ids = news_similarities[news_ID]
               for j, similar_news_id in enumerate(similar_news_ids):
                  if n >= self.similarity_num:
                      break
                  if news_ID != similar_news_id:
                      self.train_samples[i][self.negative_sample_num + n + 1] = self.news_ID_dict[similar_news_id]   # �����Ƶ����ż���ѵ��������
                      n=n+1
                  #print(f"{news_ID}'s similarty {similar_news_id}---{self.news_ID_dict[similar_news_id]}")
            else:
               #print(f"{news_ID} is not in similarity file")
               self.miss_num=self.miss_num+1
               for j in range(self.similarity_num):
                  self.train_samples[i][self.negative_sample_num + j + 1] = train_behavior[3]
            # ****************************************************************************** #
        print(f"{self.miss_num} news are lossing")
        end_time = time.time()
