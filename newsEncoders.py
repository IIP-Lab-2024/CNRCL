...
from augument_title import augument_title
from augument_content import augument_content


def expand_tensors(tensors, dim=1):
    expanded_tensors = torch.cat((tensors, tensors.clone()), dim=dim)
...
class Gate(nn.Module):
        ...
        def forward(self, news_rep_0, news_rep_1, news_rep_2):
             #news_rep_0 = F.relu(self.fc(news_rep_0))
             #news_rep_1 = F.relu(self.fc(news_rep_1))
             #news_rep_2 = F.relu(self.fc(news_rep_2))
             gate_weights_0 = torch.sigmoid(self.gate(F.relu(self.fc(news_rep_0))))
             gate_weights_1 = torch.sigmoid(self.gate(F.relu(self.fc(news_rep_1))))
             gate_weights_2 = torch.sigmoid(self.gate( F.relu(self.fc(news_rep_2))))
             
             gate_weights = torch.stack([gate_weights_0, gate_weights_1, gate_weights_2])
             gate_weights = F.softmax(gate_weights, dim=0)
             news_representation = gate_weights[0] * news_rep_0 + gate_weights[1] * news_rep_1 + gate_weights[2] * news_rep_2

             #news_representation = gate_weights_0 * news_rep_0 + gate_weights_1 * news_rep_1 + gate_weights_2 * news_rep_2
             return news_representation




class CNRCL(NewsEncoder):
    def __init__(self, config: Config):
        super(CNRCL, self).__init__(config)
        self.max_title_length = config.max_title_length
        self.max_content_length = config.max_abstract_length
        self.word_embedding_dim = config.word_embedding_dim
        self.hidden_dim = config.hidden_dim
        self.news_embedding_dim = config.hidden_dim *4+100
        ...
        #self.title_lstm = nn.GRU(self.word_embedding_dim, self.hidden_dim, batch_first=True, bidirectional=True)
        #self.content_lstm = nn.GRU(self.word_embedding_dim, self.hidden_dim, batch_first=True, bidirectional=True)
        ...
        self.category_cross_attention = ScaledDotProduct_CandidateAttention(self.hidden_dim * 2, self.category_embedding_dim, config.attention_dim)
        self.subcategory_cross_attention = ScaledDotProduct_CandidateAttention(self.hidden_dim * 2, self.category_embedding_dim , config.attention_dim)

        self.gate=Gate(config.hidden_dim)
    ...
    def initialize(self):
        ...
        self.category_cross_attention.initialize()
        self.subcategory_cross_attention.initialize()
        #self.attention.initialize()
        #self.gate.initialize()

    def forward(self, title_text, title_mask, title_entity, content_text, content_mask, content_entity, category, subCategory, user_embedding):
         
        #print('title_text shape:', title_text.shape)
        #print('title_text:', title_text[:,:5,:])
        #print(m)
        category_representation = self.category_embedding(category)           # [batch_size, news_num, category_embedding_dim]
