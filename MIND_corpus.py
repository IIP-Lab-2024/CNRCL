...
class MIND_Corpus:
    ...
    def __init__(self, config: Config):
        ...
        with open('news_ID-%s.json' % config.dataset, 'r', encoding='utf-8') as news_ID_f:
            ...
            #####
            self.ID_news_dict = {v: k for k, v in self.news_ID_dict.items()}
            #####
        ...
        with open('user_history_graph-' + str(config.max_history_num) + ('' if config.no_self_connection else '-self') + ('' if config.no_adjacent_normalization else '-normalize-' + config.gcn_normalization_type) + '-' + config.dataset + '.pkl', 'rb') as user_history_graph_f:
            ...
            self.similarity_num = config.similarity_num
        # meta data
        ...
        similarity_results = {}
        if os.path.exists('similarity_results_history_small.pkl'):
            with open('similarity_results_history_small.pkl', 'rb') as f:
                similarity_results = pickle.load(f)
        else:
            print("Cannot find the pre-calculated similarity file. Please check again.")
            exit()
            ...
            for behavior_index, line in enumerate(train_behaviors_f):
                ...
                if len(history) != 0:
                    ...
                    for non_click_impression in non_click_impressions:
                          if impression_ID in similarity_results and non_click_impression in similarity_results[impression_ID]:
                              similarity = similarity_results[impression_ID][non_click_impression]
                          else:
                              print("Cannot find the similarity for impression_ID: {}, non_clicked_news: {}.".format(impression_ID, non_click_impression))
                              similarity = 0  # or other default value
                          similarities.append((non_click_impression, similarity))
                    similarities.sort(key=lambda x: x[1])

                    least_similar_non_click_impressions = [x[0] for x in similarities]
                    least_similar_non_click_impressions_representations = [self.news_ID_dict[id] for id in least_similar_non_click_impressions]
                    
                    for click_impression in click_impressions:
                       self.train_behaviors.append([self.user_ID_dict[user_ID], user_history, user_history_mask, self.news_ID_dict[click_impression],least_similar_non_click_impressions_representations, behavior_index])
                else:
                    for click_impression in click_impressions:
                        non_click_impressions_representations = [self.news_ID_dict[id] for id in non_click_impressions]
                        self.train_behaviors.append([self.user_ID_dict[user_ID], [0 for _ in range(self.max_history_num)], np.zeros([self.max_history_num], dtype=bool), self.news_ID_dict[click_impression], non_click_impressions_representations, behavior_index])
