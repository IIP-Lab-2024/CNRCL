...
class Trainer:
    def __init__(self, model: nn.Module, config: Config, mind_corpus: MIND_Corpus, run_index: int):
        ...
        self.lr_decay_multiplier = 0.9
        self.lr_decay_schedule = [4, 7, 10, 13]  # the epochs at which the learning rate should be decayed
        self.current_decay = 1.0  # track the current decay multiplier
    ...
    def lr_decay(self):
        for group in self.optimizer.optimizer.param_groups:
           group['lr'] = group['lr'] * self.current_decay
           print(f'Learning rate is decayed. The new learning rate is {group["lr"]}.') 
    ...
    def train(self):
        ...
        for e in tqdm(range(1, self.epoch + 1)):
            self.train_dataset.negative_sampling(e)
            train_dataloader = DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True, num_workers=0, pin_memory=True)
            if e in self.lr_decay_schedule:
                self.current_decay *= self.lr_decay_multiplier
                self.lr_decay()
                print(f'At epoch {e}, the learning rate is decayed.')
            model.train()
            epoch_loss = 0
            num = 0
            for (user_ID, user_category, user_subCategory, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, user_history_mask, user_history_graph, user_history_category_mask, user_history_category_indices, news_category, news_subCategory, news_title_text, news_title_mask, news_title_entity, news_content_text, news_content_mask, news_content_entity) in train_dataloader:
                ...
                logits,logits_unsum = model(user_ID, user_category, user_subCategory, user_title_text, user_title_mask, user_title_entity, user_content_text, user_content_mask, user_content_entity, user_history_mask, user_history_graph, user_history_category_mask, user_history_category_indices,news_category, news_subCategory, news_title_text, news_title_mask, news_title_entity, news_content_text, news_content_mask, news_content_entity) # [batch_size, 1 + negative_sample_num]
                ...
                supcon_loss_nlp = NewsRecSupConLoss_with_neg_pos(temperature=0.07, base_temperature=0.07).to('cuda')
                sloss = supcon_loss_nlp(logits_unsum,4)
                losses = [loss,sloss]
            ...
            # validation
            print(self.dev_res_dir + '/' + model.model_name + '-' + str(e) + '.txt')
